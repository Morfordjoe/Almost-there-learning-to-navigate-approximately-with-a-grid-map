...
#Has a continuous output from neural networks
...
from pandas import *
from NN_functions_2023 import test, format_angles_0_1, format_angles_0_2pi, format_np_array, make_compile_NN_model
...
for rn in range(no_runs):
    ...
    train_ang_01 = format_angles_0_1(train_angles, ind_angle_dev)
    ...
    model = make_compile_NN_model(grid)
    ...
    test_pred_0_2pi = format_angles_0_2pi(test_pred, ind_angle_dev)
    ...
    for sz in range(1, len(training_test_sizes)):
        ...
        y = train_ang_01[training_test_sizes[sz-1]:training_test_sizes[sz]],
        validation_split=0,
        batch_size = 1,
        epochs = 1,
        )
...
for sz1 in range(len(training_test_sizes)):
    ...
    if file_out:
        file_name = "NN_continuous_2023_out/NN2023out_" + gradient1 + "_" + gradient2 + "_trainsize_" + str(dict_in) + "_n-nets_" + str(no_runs) + "_" + str(date.today()) + ".csv"
